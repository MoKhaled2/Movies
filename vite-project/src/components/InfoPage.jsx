...
export default function InfoPage() {
    ...
    useEffect(() => {
        ...
        setLoading(true)
        setError("")
        fetch(`https://api.themoviedb.org/3/movie/${id}?api_key=${apiKey}&append_to_response=videos,credits`)
            .then(res => {
                if (!res.ok){
        ...
        return (
            <main className="container mt-5 pt-5" aria-busy="true">
                <div className="d-flex justify-content-center align-items-center" style={{ minHeight: "50vh" }}>
                    <div className="spinner-border text-warning" role="status" aria-label="Loading"></div>
                </div>
            </main>
        )
    }
            ...
            <section className="p-3 p-md-4 mb-4 text-white" style={backdropStyle}>
                <div className="row g-4 align-items-stretch">
                    <div className="col-12 col-md-3">
                        <img
                            src={movie.poster_path ? `${imgBasePoster}${movie.poster_path}` : ""}
                    ...
                    <div className="col-12 col-md-9 d-flex flex-column">
                        <header>
                            ...
                            <p className="mb-2">
                                <span className="me-2 text-warning fw-semibold">{userScore}% User Score</span>
                                {movie.release_date ? <span className="me-2">• {movie.release_date}</span> : null}
                                {runtimeLabel ? <span className="me-2">• {runtimeLabel}</span> : null}
                            </p>
                            <div className="mb-3">
                                {(movie.genres || []).map(g => (
                                    <span key={g.id} className="badge text-bg-light me-2 mb-2">{g.name}</span>
                                ))}
                            </div>
                        </header>
                        ...
                        <div className="d-flex align-items-center mb-3">
                            ...
                            <Link to="/" className="btn btn-outline-light ms-auto">← Back</Link>
                        </div>
            ...
            <section className="mb-4">
                ...
                <div className="row g-3">
                    {(movie.credits?.cast || []).slice(0, 8).map((person) => (
                        <article key={person.cast_id || person.credit_id || person.id} className="col-6 col-sm-4 col-md-3 col-lg-2">
                            <div className="card h-100 border-0 shadow-sm">
                                <img
                                    src={person.profile_path ? `${imgBasePoster}${person.profile_path}` : "https://via.placeholder.com/300x450?text=No+Image"}
                                    alt={person.name}
                                    className="card-img-top rounded-top"
                                />
                                <div className="card-body p-2">
                                    <h3 className="h6 mb-1" style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{person.name}</h3>
                                    <p className="text-muted small mb-0" style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{person.character}</p>
                                </div>
                            </div>
                        </article>
                    ))}
                        ...
                        <dl className="row mb-0">
                            ...
                            {typeof movie.revenue === 'number' && movie.revenue > 0 ? (
                                <>
                                    ...
                                    <dd className="col-8">{new Intl.NumberFormat(undefined, { style: 'currency', currency: 'USD', maximumFractionDigits: 0 }).format(movie.revenue)}</dd>
                                </>
                            ) : null}
                        </dl>
                    </div>
                    ...
                    <div className="p-3 border rounded h-100 bg-light">
                        ...
                        <div className="d-flex flex-wrap gap-2">
                            {(movie.production_companies || []).map(pc => (
                                <span key={pc.id} className="badge text-bg-secondary">{pc.name}</span>
                            ))}
                        </div>
                    </div>
                </div>
